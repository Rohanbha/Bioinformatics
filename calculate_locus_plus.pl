...
my $target_file = 'Arab_eValue_filtered_with_chr_plus.txt';
my $gff_file = 'Athaliana_447_Araport11.gene_exons_PlusStrand.gff3';
my $output_file = 'Finalplusoutput.txt';
...
open(our $of, '>>', 'Finalplusoutput.txt') or die "Can't read file output.txt [$!]\n";
while (our $line1 = <$tf>) {                                              #Read each row
    ...
    while (our $line2 = <$gf>) {
	    ...
	    if ($ncbi2 eq $ncbi) {		    #Select Med6
		    ...
		    open(our $of, '>>', 'Finalplusoutput.txt') or die "Can't read file Finalplusoutput.txt [$!]\n";
